...
const Filling = (props) => {
  ...
  const [briefShow, setBriefShow] = useState(false);
  ...
  const submit = (e) => {
    ...
    for (let i = 0; i < formFields.length; i++) {
      ...
      method['Step ' + parseInt(j)] = {
        ...
        video_url: formFields[i].video_url,
      }
  ...
  const updateStep = (e) => {
    ...
    db.collection("recipe_method").doc(page.code).update({
      method: method
    });
    // setPage(page.filling_code)
    handleClose()
    ...
    const unsub = db.collection("recipe_method").doc(page.code).onSnapshot((snapshot) => {
      ...
      try {
        ...
        for (let i = 0; i < sortArr.length; i++) {
          ...
          <img src={sortArr[i].video_url}  height="100px" />
        </div>
  ...
  const handleUrl = (url, index,docs) => {
 
    console.log(url);
    console.log(index);
    console.log(docs);

    docs[index]['video_url'] = url;
  ...
  //const ImageUpload = () => {
    ...
    const handleGifUpload = (files, index) => {
      let data = [...formFields];
      //data[index][event.target.name] = event.target.value;
      ...
      handleUpload(data,index,files);
    }
  
    const handleUpload = (doc,index,files) => {
      if (!files) {
      ...
      storage.ref(`/recipe_method/${files.name}`).put(files);

      
  
      var starsRef = storage.ref().child(`/recipe_method/${files.name}`);
    ...
    <div className="m-3">
      ...
      <Modal animation={true}
        ...
        <input accept="image/gif"
          ...
          id="select-image"
          style={{ display: "none" }}
          onChange={e => handleGifUpload(e.target.files[0],index)} />
      </label>
      ...
      {form.video_url && (
        <div>
          ...
          <img src={form.video_url}  height="100px" />
        </div>
        ...
        <Modal.Footer>
          ...
          <OverlayTrigger className="me-auto" placement="top"
            ...
            <Button variant="info" style={{ width: "15vw" }} onClick={updateStep}>
              <h5 className="mt-1">Update Step</h5>
            </Button>
          </OverlayTrigger>
        </Modal.Footer>
      </Modal>
